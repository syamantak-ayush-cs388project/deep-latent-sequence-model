...
def load_para_data(params, data):
    ...
    for (lang1, lang2), paths in params.para_dataset.items():
        ...
        for name, path in zip(['train', 'valid', 'test'], paths):
            ...
            # load data
            data1 = load_binarized(path.replace('X', lang1), params)
            data2 = load_binarized(path.replace('X', lang2), params)
...
def check_all_data_params(params):
    ...
    for (lang1, lang2), (train_path, valid_path, test_path) in params.para_dataset.items():
        ...
        assert train_path == '' or os.path.isfile(train_path.replace('X', lang1))
        assert train_path == '' or os.path.isfile(train_path.replace('X', lang2))
        assert os.path.isfile(valid_path.replace('X', lang1))
        assert os.path.isfile(valid_path.replace('X', lang2))
        assert os.path.isfile(test_path.replace('X', lang1))
        assert os.path.isfile(test_path.replace('X', lang2))
